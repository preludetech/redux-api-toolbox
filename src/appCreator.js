...
export function createReduxApp({
  ...
  const creators = {
    start: ({ data, successDispatchActions }) => ({
    ...
    maybeStart: ({ data, successDispatchActions }) => ({
      type: types.ADD_NEW_START,
      data,
      successDispatchActions,
      force: false,
    ...
    _start: ({ data, callIndex, successDispatchActions }) => {
  ...
  const operations = {
    ...
    maybeStart: params => {
      const { data, successDispatchActions } = params;
